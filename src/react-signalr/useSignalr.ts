...
type SendFunction = (methodName: string, ...args: unknown[]) => Promise<void>;
...
interface UseSignalrHookResult {
   ...
   * @param args - The arguments used to invoke the server method. If no arg is passed or the value passed is undefined, nothing will be sent to the SignalR endpoint.
   *
...
export function getOrSetupConnection(
  hubUrl: string,
...
export function useSignalr(
    ...
    (methodName: string, ...args: unknown[]) => {
      return connection$
        .pipe(
          ...
          take(1),
          // use the connection
          switchMap(connection => connection.send(methodName, ...args))
        )
        .toPromise();
    },
    [connection$]
  ...
  const invoke = useCallback<InvokeFunction>(
    <TResponse>(methodName: string, ...args: unknown[]) => {
      return connection$
        .pipe(
          // only take the current value of the observable
          take(1),
          // use the connection
          switchMap(connection => connection.invoke<TResponse>(methodName, ...args))
        )
